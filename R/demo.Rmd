...
type_ops <- c("cargo ship","tanker","passenger ship") 
...
traffic <- traffic %>% dplyr::filter(length > 0,
                                     sog > 0) 
...
mrs <- c() # simple tally of imminent encounters
```
...
for(b in 1:iterations){
  ...
  # Store detailed results from each run within the simulation
  df <- rbind(df, mr)
  
  # Note number of imminent encounters that occurred in this iteration
  ieb <- length(which(mr$proximity_m <= 0)) 
  mrs <- c(mrs,ieb)
...
```{r,eval=FALSE}
encounters <- which(df$proximity_m == 0)
encounter_rate <- length(encounters) / nrow(df)
encounter_rate
```
...
hist(mrs, main='Imminent encounter rates') 

hist(df$proximity_m,
     breaks=seq(0,(1.1*max(df$proximity_m)),length=30),
     main="Nearest proximity during each co-occurrence")

hist(df$whale_hdg,
     breaks=seq(0,(1.1*max(df$whale_hdg)),length=30),
     main="Whale heading at nearest proximity")

hist(df$ship_hdg,
     breaks=seq(0,(1.1*max(df$ship_hdg)),length=30),
     main="Vessel heading at nearest proximity")

hist(df$speed_ratio,
     breaks=seq(0,(1.1*max(df$whale_sog)),length=30),
