#########################################################
#########################################################
# ENCOUNTER RATE - SENSITIVITY SIMULATION - L WHALE
#########################################################
#########################################################
...
filecore <- 'l-whale'
l.whales = c(9,12,15,18,21,24,27,30)
vars <- l.whales
...
mrs <- data.frame()
vals <- c(9,12,15,18,21,24,27,30) ; vals

#pdf("../figures/sim/lwhale.pdf",width=4.5,height=4.5)
par(mar=c(4.2,4.2,.5,.5))
plot(1,type="n",ylim=c(0,10),xlim=c(9,30),axes=FALSE,
...
for(i in vals){
  mr <- readRDS(file=paste0("../results/sim/lwhale-",i,".rds"))
  mr$encounters
  ...
  points(x=jitter(mri$val,amount=.4),
...
pval(val1=mrs$enc[mrs$val==12],
     val2=mrs$enc[mrs$val==30])
